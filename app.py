...
def seed_data():
    # Load sample_data.json if collections empty
    try:
...
def ingest():
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            file.save(path)
        db.ingest.insert_one({'title': title, 'description': description, 'filename': filename, 'timestamp': datetime.datetime.utcnow()})
        return redirect(url_for('ingest'))
    items = list(db.ingest.find({}, {'_id':0}).sort('timestamp', -1))
    return render_template('ingest.html', items=items)
...
def otolith():
    if request.method == 'POST':
        ...
        if file and file.filename:
            ...
            path = os.path.join(UPLOAD_FOLDER, filename)
            file.save(path)
        db.otolith.insert_one({'species': species, 'length_mm': float(length_mm) if length_mm else None, 'filename': filename, 'timestamp': datetime.datetime.utcnow()})
...
def edna():
    if request.method == 'POST':
        ...
        db.edna.insert_one({'sample_id': sample_id, 'species_detected': species_arr, 'timestamp': datetime.datetime.utcnow()})
...
def api_visualization_data():
    ingestion = list(db.ingest.find({}, {'_id':0}))
    otolith = list(db.otolith.find({}, {'_id':0}))
    edna = list(db.edna.find({}, {'_id':0}))
    ingestion_count = db.ingest.count_documents({})
    ...
    for doc in edna:
        for sp in doc.get('species_detected', []):
            species_counts[sp] = species_counts.get(sp,0) + 1
    # timeseries by ingest date (YYYY-MM-DD)
    ts = {}
    ...
    timeseries = sorted([{'date':k,'count':v} for k,v in ts.items()], key=lambda x: x['date'])
    return jsonify({'ingestion_count': ingestion_count, 'otolith_count': otolith_count, 'edna_count': edna_count, 'species_counts': species_counts, 'timeseries': timeseries})
